...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    Plotly.newPlot("bubble", bubbleData, bubbleLayout); 
       ...
       // 3.1 Create a variable that holds the metadata filters the metadata array for the object with the desired sample number.
       var metadata = data.metadata;
       var gauge = metadata.filter(metaObj => metaObj.id == sample);
 
       // 3.2 Create a variable that holds the first metadata in the array.
       var gaugeDisp = gauge[0];
 
       // 3.3 Create a variable that converts the washing frequency to a floating point number.
       var wfreqs = gaugeDisp.wfreq;
